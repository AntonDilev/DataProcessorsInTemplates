...
Процедура ИнициализироватьПараметрыКонтекста() Экспорт
	...
	Если Отладка Или ЭтоТолстыйКлиент() Тогда
		ПараметрыКонтекста.АдресМодуляИнтеграции = КаталогОбработки + ИмяМакетаИнтеграции + ".epf";
	Иначе
		ПараметрыКонтекста.АдресМодуляИнтеграции = ПодключитьОбработкуИзМакета(ИмяМакетаИнтеграции);
...
//
// Параметры:
//  ИмяМакетаОбработки	 - Строка	 - имя макета в котором хранится обработка в виде двоичных данных
// 
// Возвращаемое значение:
//  Строка - Адрес подключенной обработки
//
Функция ПодключитьОбработкуИзМакета(ИмяМакетаОбработки)
